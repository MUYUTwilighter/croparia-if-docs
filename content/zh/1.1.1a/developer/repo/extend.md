---
title: 扩充 Repo API
desc: 介绍如何围绕 Repo API 扩展新的存储方式，包括 Resource API 注册、RepoProxy 平台包装，以及 ProxyProvider 的注册与发现流程。
keywords:
  - Croparia IF
  - Repo API
  - RepoProxy
...
这里有一个很值得利用的细节：`RepoProxy` 本身继承自 `DelegateRepo`，因此你传入的如果已经是一个经过 `lockAccept(...)`、`lockConsume(...)` 或 `lock(...)` 处理的视图，平台代理层会直接继承这层限制，而不需要再在平台实现里手动补一套槽位过滤逻辑。
